...
class Review4dDialog(gui.GeDialog):
    ...
    def CreateLayout(self):
        ...
        if self.GroupBegin(
            self.GROUP_SETTINGS, c4d.BFH_SCALEFIT, cols=3, rows=3, title="Settings"
        ):
            self.GroupBorder(c4d.BORDER_IN)
            self.GroupBorderSpace(4, 4, 4, 4)
        ...
        if self.post_renderers:
            if self.GroupBegin(
                self.GROUP_POSTRENDER, c4d.BFH_SCALEFIT, cols=2, title="Post Render"
            ):
                self.GroupBorder(c4d.BORDER_IN)
                self.GroupBorderSpace(4, 4, 4, 4)
        ...
        if self.GroupBegin(
            self.GROUP_BUTTONS, c4d.BFH_RIGHT | c4d.BFV_SCALE | c4d.BFV_BOTTOM, title=""
        ):
            self.GroupBorderNoTitle(c4d.BORDER_GROUP_OUT)
            self.GroupBorderSpace(4, 10, 4, 4)
            self.AddButton(
                self.BUTTON_RENDER_SETTINGS, c4d.BFH_RIGHT, name="Edit Review Settings"
            )
    ...
    def GetValues(self):
        values = {
            "preset": review4d.get_path_preset(self.GetInt32(self.COMBO_PRESET)).label,
    ...
    def OnPathChanged(self, path):
        ...
        for preset in review4d.get_path_presets():
            ...
            with review4d.suppress_messages(self):
                self.SetInt32(self.COMBO_PRESET, review4d.get_path_preset("Custom").id)
